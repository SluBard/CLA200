...
struct MinScore {
    ...
    bool operator ()(int a) const { return a >= minimum;  }
};
...
int main()
{
    ...
    if (scores.empty()) {                 // vector is empty
        ...
        int min = 90;
        cout << "Using Struct\n";
        cout << "Scores " << min << " or above: " << count_if(scores.begin(),scores.end(),MinScore(min)) << endl;
        min = 80;
        cout << "Scores " << min << " or above: " << count_if(scores.begin(), scores.end(), MinScore(min)) << endl;

        min = 90;
        cout << "Using lambda\n";
        auto minscore = [&min](int a)->bool {return a >= min; };
        cout << "Scores " << min << " or above: " << count_if(scores.begin(), scores.end(), minscore) << endl;
        min = 80;
        cout << "Scores " << min << " or above: " << count_if(scores.begin(), scores.end(), minscore) << endl;
