...
int main()
{
    ...
    while (!valid_email) {
        ...
        if (dot_index - at_index == 1) {
            cout << "The domain name of the email address must include at least one character.\n";
            valid_email = false;
        }


        // make sure TLD domain has 2 or 3 characters
        int length_of_domain = email.size() - dot_index - 1;
        if (length_of_domain < 2 || length_of_domain > 3) {
            cout << "The TLD domain name of the email must have two or three characters.\n";
            valid_email = false;
        }
