...
int main()
{
    ...
    //sort(scores.begin(), scores.end(), [](int a, int b)->bool {return b < a; });
    for_each(scores.begin(), scores.end(), display);
    //for_each(scores.begin(), scores.end(), [](int a)->void {cout << a << " "; });
    cout << endl;
    ...
    if (scores.empty()) {                 // vector is empty
        ...
        //int total = 0;
        int total = accumulate(scores.begin(), scores.end(), 0.0);
